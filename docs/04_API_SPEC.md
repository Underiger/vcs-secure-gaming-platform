...
- **命名空間**：`/`（預設）
- **transports**：生產環境僅 `['websocket']`（node:cluster 多 worker 無 sticky
  session，polling 後續請求可能落到不認得 session 的 worker → `Session ID unknown`
  立即斷線；2026-07-05 修正，見 PROJECT_STATE.md 同日條目）；開發環境保留
