...
**關鍵路徑（已全數完成）**：M01 → M02 → M04 → M06 → M11 → M14 → M25 → M26 → M27 → M28（約 14.0 人天估算）；C/E/F/G 階段內多數工作已與關鍵路徑並行排程完成。**M29** 接續於 v1.0.0 發布之後，依賴 M06（HMAC/限流）/M07（wallet）/M11（Slot 服務模式）/M15（roulette leader lock → round-lock 同款前例），屬 Phase 2 第一批、不影響原關鍵路徑。**M30** 接續於 M29 之後，依賴 M06（HMAC/限流）/M07（wallet）/M29（射龍門 open→bet 單步原子金流同款前例），屬 Phase 2 第二批、同樣不影響原關鍵路徑。
